...
<html>
	...
	<body>
		...
		<!--  
	 		...
	 		3. 由于struts.xml已配置Spring mvc托管对象，因而服务器扫描Spring 配置文件applicationContext.xml，Spring 预先记住注入绑定关系：
	 			...
	 			Spring准备创建regiter的对象：com.xyb.sshdemo.action.RegisterAction
	 		4. 因com.xyb.sshdemo.action.RegisterAction存在form实体属性且提供get/set方法
	 			(若form实体是通过new form实体()得到的，只需要提供set方法即可)
	 			从而，Spring 先创建了form实体，将浏览器提交的表单域模型数据(对象.属性)保存在form实体中，即form实体获取了浏览器提交的表单数据
	 		5. 但是，因为存在表单数据校验配置文件：action动作名-validation.xml，从而，struts2将根据校验文件进行匹配form实体中保存的属性数据
	 			匹配通过，那么Spring就创建regiter的对象：com.xyb.sshdemo.action.RegisterAction，执行默认excute()方法，继续步骤6
	 			匹配不通过，那么Struts2将校验文件提示信息响应浏览器，SpringC不会创建regiter的对象，本轮http请求结束
	 		6. 由于action动作实例满足Spring配置关系，因而Spring创建sevice实例，POJOdao实例、sessionFactory实例、configLocation实例，
